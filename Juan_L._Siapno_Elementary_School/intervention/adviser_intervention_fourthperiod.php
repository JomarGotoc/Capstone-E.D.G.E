...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE $table SET status=?, gname=?, number=?, notes=?, intervention=?, topic=?, advice=? WHERE lrn=? AND quarter = '4' AND school = 'Juan L. Siapno Elementary School'";
...
if (isset($_GET['lrn']) && isset($_GET['classification'])) {
    ...
    foreach ($tables as $table) {
        $query = "SELECT gname, number, grade, section FROM $table WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '4' AND school = 'Juan L. Siapno Elementary School'";
