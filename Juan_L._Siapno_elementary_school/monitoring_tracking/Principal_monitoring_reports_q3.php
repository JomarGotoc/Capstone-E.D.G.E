...
<?php
    ...
    $sqlEnglish = "SELECT COUNT(*) AS total FROM academic_english WHERE  school = 'Juan L. Siapno Elementary School'";
    $resultEnglish = $conn->query($sqlEnglish);
    $rowEnglish = $resultEnglish->fetch_assoc();
    $totalEnglish = $rowEnglish['total'];

    $sqlFilipino = "SELECT COUNT(*) AS total FROM academic_filipino WHERE  school = 'Juan L. Siapno Elementary School'";
    $resultFilipino = $conn->query($sqlFilipino);
    $rowFilipino = $resultFilipino->fetch_assoc();
    $totalFilipino = $rowFilipino['total'];

    $sqlNumeracy = "SELECT COUNT(*) AS total FROM academic_numeracy WHERE  school = 'Juan L. Siapno Elementary School'";
    $resultNumeracy = $conn->query($sqlNumeracy);
    $rowNumeracy = $resultNumeracy->fetch_assoc();
    $totalNumeracy = $rowNumeracy['total'];

    $sqlBehavioral = "SELECT COUNT(*) AS total FROM behavioral WHERE  school = 'Juan L. Siapno Elementary School'";
    $resultBehavioral = $conn->query($sqlBehavioral);
    $rowBehavioral = $resultBehavioral->fetch_assoc();
    $totalBehavioral = $rowBehavioral['total'];

    $sqlEnglishResolved = "SELECT COUNT(*) AS total FROM academic_english WHERE status = 'resolved' AND school = 'Juan L. Siapno Elementary School'";
    $resultEnglishResolved = $conn->query($sqlEnglishResolved);
    $rowEnglishResolved = $resultEnglishResolved->fetch_assoc();
    $totalEnglishResolved = $rowEnglishResolved['total'];

    $sqlFilipinoResolved = "SELECT COUNT(*) AS total FROM academic_filipino WHERE status = 'resolved'AND school = 'Juan L. Siapno Elementary School' ";
    $resultFilipinoResolved = $conn->query($sqlFilipinoResolved);
    $rowFilipinoResolved = $resultFilipinoResolved->fetch_assoc();
    $totalFilipinoResolved = $rowFilipinoResolved['total'];

    $sqlNumeracyResolved = "SELECT COUNT(*) AS total FROM academic_numeracy WHERE status = 'resolved' AND school = 'Juan L. Siapno Elementary School'";
    $resultNumeracyResolved = $conn->query($sqlNumeracyResolved);
    $rowNumeracyResolved = $resultNumeracyResolved->fetch_assoc();
    $totalNumeracyResolved = $rowNumeracyResolved['total'];

    $sqlBehavioralResolved = "SELECT COUNT(*) AS total FROM behavioral WHERE status = 'resolved' AND school = 'Juan L. Siapno Elementary School'";
...
<?php
    ...
    function countOccurrences($conn, $table, $grade) {
        $sql = "SELECT COUNT(*) AS count FROM $table WHERE grade = '$grade' AND quarter = '3' AND school = 'Juan L. Siapno Elementary School'";
