...
if (isset($_POST['update'])) {
    ...
    $sql_update_status = "UPDATE $table SET status=? WHERE lrn=? AND quarter IN ('2', '3', '4') AND school = 'Federico N. Ceralde School Integrated School'";
    ...
    if ($status == 'Resolved') {
        ...
        $sql_delete = "DELETE FROM $table WHERE lrn=? AND quarter IN ('2', '3', '4') AND school = 'Federico N. Ceralde School Integrated School'";
    ...
    $sql = "UPDATE $table SET status=?, gname=?, number=?, notes=?, intervention=?, topic=?, advice=? WHERE lrn=? AND quarter = '1' AND school = 'Federico N. Ceralde School Integrated School'";
...
if (isset($_GET['lrn']) && isset($_GET['classification'])) {
    ...
    foreach ($tables as $table) {
        $query = "SELECT gname, number, grade, section FROM $table WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '1' AND school = 'Federico N. Ceralde School Integrated School'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if (!empty($row['gname'])) {
                ...
                header("Location: adviser_intervention_firstperiod_view.php?lrn=$lrn&classification=$classification&grade=$grade&section=$section&employment_number={$_GET['employment_number']}");
                exit(); 
            }
        }
    }
