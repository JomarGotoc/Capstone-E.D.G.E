<?php
    ...
    if (isset($_GET['lrn']) && isset($_GET['classification'])) {
        ...
        $english_query = "SELECT fullname, grade, section, gname, number, status, notes, intervention, topic, advice FROM academic_english WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '1' AND school = 'Federico N. Ceralde School Integrated School'";

        // Query to fetch data from academic_filipino table
        $filipino_query = "SELECT fullname, grade, section, gname, number, status, notes, intervention, topic, advice FROM academic_filipino WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '1' AND school = 'Federico N. Ceralde School Integrated School'";

        // Query to fetch data from academic_numeracy table
        $numeracy_query = "SELECT fullname, grade, section, gname, number, status, notes, intervention, topic, advice FROM academic_numeracy WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '1' AND school = 'Federico N. Ceralde School Integrated School'";

        // Query to fetch data from behavioral table
        $behavioral_query = "SELECT fullname, grade, section, gname, number, status, notes, intervention, topic, advice FROM behavioral WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '1' AND school = 'Federico N. Ceralde School Integrated School'";
...
if(isset($_POST['print'])) {
    ...
    $words = explode('_', $filename);
        ...
        $redirect_url = "adviser_intervention_print.php?classification=$classification&lrn=$lrn&employment_number=$employment_number&filename=$filename1&quarter=1";
