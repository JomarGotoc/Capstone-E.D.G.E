...
if (isset($_GET['lrn']) && isset($_GET['fullname']) && isset($_GET['status']) && isset($_GET['grade']) && isset($_GET['section']) && isset($_GET['classification'])) {
    ...
    $sql = "SELECT gname, number FROM $table WHERE lrn = '$lrn' AND quarter = 1 AND school = 'Salapingao Elementary School' LIMIT 1";
...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE $table SET status=?, gname=?, number=?, notes=?, intervention=?, topic=?, advice=? WHERE lrn=? AND quarter = '3' AND school = 'Salapingao Elementary School'";
    ...
    if ($stmt->execute()) {
        ...
        if ($status === 'Resolved') {
            $school = "Salapingao Elementary School";
        ...
        $sql_update_quarter_4 = "UPDATE $table SET status=? WHERE lrn=? AND quarter IN ('3', '4') AND school = 'Salapingao Elementary School'";
...
if (isset($_GET['lrn']) && isset($_GET['classification'])) {
    ...
    foreach ($tables as $table) {
        $query = "SELECT gname, number, grade, section FROM $table WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '3' AND school = 'Salapingao Elementary School'";
