<?php
    ...
    if (count($words) >= 4) {
        $secondWord = $words[1];
        $fourthWord = $words[3];
        $sql = "SELECT employment_number, fullname FROM adviser WHERE grade = '$secondWord' AND section = '$fourthWord' AND school = 'Tambac Elementary School'";
...
foreach ($tables as $table) {
    $sql = "SELECT COUNT(*) AS count FROM $table WHERE grade = '$secondWord' AND section = '$fourthWord' AND school = 'Tambac Elementary School'";
...
<?php
    ...
    function fetchTable($conn, $tableName, $grade, $section) {
        ...
        $sql = "SELECT lrn, fullname, classification, grade, section, status FROM $tableName WHERE grade = ? AND section = ? AND quarter = 1 AND school = 'Tambac Elementary School'";
