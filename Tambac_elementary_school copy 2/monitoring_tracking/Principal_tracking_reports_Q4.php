<?php
    ...
    $sql = "SELECT grade, section, fullname FROM adviser Where school = 'Tambac Elementary School'";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            if ($tableExists->num_rows > 0) {
                $countQuery = "SELECT COUNT(*) AS lrn_count FROM $newvalue WHERE school = 'Tambac Elementary School'";
            ...
            $academicEnglishQuery = "SELECT COUNT(*) AS academic_english_count FROM academic_english WHERE grade = '{$row['grade']}' AND section = '{$row['section']}' AND quarter = 4 AND school = 'Tambac Elementary School'";
            $academicFilipinoQuery = "SELECT COUNT(*) AS academic_filipino_count FROM academic_filipino WHERE grade = '{$row['grade']}' AND section = '{$row['section']}' AND quarter = 4 AND school = 'Tambac Elementary School'";
            $academicNumeracyQuery = "SELECT COUNT(*) AS academic_numeracy_count FROM academic_numeracy WHERE grade = '{$row['grade']}' AND section = '{$row['section']}' AND quarter = 4 AND school = 'Tambac Elementary School'";
            $behavioralQuery = "SELECT COUNT(*) AS behavioral_count FROM behavioral WHERE grade = '{$row['grade']}' AND section = '{$row['section']}' AND quarter = 4 AND school = 'Tambac Elementary School'";
...
<?php
    ...
    foreach ($tables as $table) {
        ...
        $sqlPars = "SELECT COUNT(*) as count FROM $table WHERE lrn IS NOT NULL AND quarter = 4 AND school = 'Tambac Elementary School'";
        ...
        $sqlResolved = "SELECT COUNT(*) as count FROM $table WHERE status = 'resolved' AND quarter = 4 AND school = 'Tambac Elementary School'";
