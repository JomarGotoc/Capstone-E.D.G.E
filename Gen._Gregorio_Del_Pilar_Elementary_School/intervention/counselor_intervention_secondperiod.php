...
if (isset($_GET['classification']) && isset($_GET['lrn'])) {
    ...
    $sql = "SELECT gname, number FROM $table WHERE lrn = ? AND quarter = 1 AND school = 'Gen. Gregorio Del Pilar Elementary School'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update"])) {
    ...
    $sql_update = "UPDATE behavioral SET 
            ...
            advice = ?,
            status = ?
            WHERE lrn = ? AND quarter = '2' AND school = 'Gen. Gregorio Del Pilar Elementary School'";
    ...
    if ($stmt_update->execute()) {
        ...
        if ($status === 'Resolved') {
            ...
            $sql_delete = "DELETE FROM behavioral WHERE lrn = ? AND quarter IN ('3', '4') AND classification = ? AND school = ?";
        ...
        $sql_update = "UPDATE behavioral SET status = ? WHERE lrn = ? AND quarter IN ('3', '4') AND school = 'Gen. Gregorio Del Pilar Elementary School'";
        $stmt_update = $conn->prepare($sql_update);
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET['lrn'])) {
        ...
        foreach ($tables as $table) {
            $sql = "SELECT COUNT(*) as count FROM $table WHERE lrn = '$lrn' AND quarter = '2' AND 
        ...
        advice IS NOT NULL AND advice <> '' AND school = 'Gen. Gregorio Del Pilar Elementary School'";
