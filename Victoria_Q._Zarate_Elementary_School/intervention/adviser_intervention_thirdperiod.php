...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE $table SET status=?, gname=?, number=?, notes=?, intervention=?, topic=?, advice=? WHERE lrn=? AND quarter = '3' AND school = 'Victoria Q. Zarate Elementary School'";
    ...
    if ($stmt->execute()) {
        ...
        $sql_update_quarter_4 = "UPDATE $table SET status=? WHERE lrn=? AND quarter IN ('3', '4') AND school = 'Victoria Q. Zarate Elementary School'";
...
if (isset($_GET['lrn']) && isset($_GET['classification'])) {
    ...
    foreach ($tables as $table) {
        $query = "SELECT gname, number, grade, section FROM $table WHERE lrn = '$lrn' AND classification = '$classification' AND quarter = '3' AND school = 'Victoria Q. Zarate Elementary School'";
