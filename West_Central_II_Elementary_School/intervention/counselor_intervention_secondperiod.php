...
if (isset($_GET['classification']) && isset($_GET['lrn'])) {
    ...
    $sql = "SELECT gname, number FROM $table WHERE lrn = ? AND quarter = 1 AND school = 'West Central II Elementary School'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update"])) {
    ...
    $sql_update = "UPDATE behavioral SET 
            ...
            advice = ?,
            status = ?
            WHERE lrn = ? AND quarter = '2' AND school = 'West Central II Elementary School'";
    ...
    if ($stmt_update->execute()) {
        ...
        $sql_update = "UPDATE behavioral SET status = ? WHERE lrn = ? AND quarter IN ('3', '4') AND school = 'West Central II Elementary School'";
        $stmt_update = $conn->prepare($sql_update);
