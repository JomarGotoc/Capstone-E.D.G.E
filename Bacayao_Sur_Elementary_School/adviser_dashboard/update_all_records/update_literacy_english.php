<?php
   ...
   include('../../../database.php');
    ...
    $sql = "SELECT lrn, fullname, gname, number, notes, topic, intervention, advice FROM academic_english WHERE grade = '$grade' AND section = '$section'";
...
<?php
    if (isset($_POST['update'])) {
        ...
        for ($i = 0; $i < count($_POST['lrn']); $i++) {
            ...
            $stmt = $conn->prepare("UPDATE academic_english SET gname = ?, number = ?, notes = ?, topic = ?, intervention = ?, advice = ?, status = ?, date = ? WHERE lrn = ?");
