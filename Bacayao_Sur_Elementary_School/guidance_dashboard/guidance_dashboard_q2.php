<?php
    ...
    $sql = "SELECT lrn, fullname, classification, grade, section, status FROM behavioral WHERE quarter ='2' AND school = 'Bacayao Sur Elementary School'";
...
<?php
include('../../database.php');
$sql = "SELECT COUNT(DISTINCT lrn) AS count FROM behavioral WHERE lrn IS NOT NULL AND lrn != '' AND school = 'Bacayao Sur Elementary School'";
...
if(isset($_POST['print'])) {
    ...
    header("Location: guidance_dashboard_print.php?employment_number=$employment_number&quarter=2");
}
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="container">
            <div class="header-content">
                ...
                <div class='name' onclick="toggleDropdown()">Stephanie Mislang</div>
                    ...
                    <a href="counselor_change_password.php?employment_number=<?php echo isset($_GET['employment_number']) ? $_GET['employment_number'] : 'default_value'; ?>&filename=<?php echo $filename ?>" style="border-top: 1px solid #ddd;">Change Password</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    while ($row = $result1->fetch_assoc()) {
        ...
        echo "<tr class='sheshable'>
                    ...
                    <th style='width:15%' class='act'>
                        <button><a href='../intervention/counselor_intervention_secondperiod.php?lrn={$row['lrn']}&fullname={$row['fullname']}&classification={$row['classification']}&grade={$row['grade']}&section={$row['section']}&status={$row['status']}&employment_number={$_GET['employment_number']}' class='updateRecordButton'>UPDATE RECORD</a></button>
                    </th>
                </tr>";
    }
}
