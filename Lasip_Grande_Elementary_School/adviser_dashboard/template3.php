<?php
    ...
    if (count($words) >= 4) {
        $secondWord = $words[1];
        $fourthWord = $words[3];
        $sql = "SELECT employment_number, fullname FROM adviser WHERE grade = '$secondWord' AND section = '$fourthWord' AND school = 'Lasip Grande Elementary School'";
...
<?php
    ...
    function fetchTable($conn, $tableName, $grade, $section) {
        ...
        $sql = "SELECT lrn, fullname, classification, grade, section, status FROM $tableName WHERE grade = ? AND section = ? AND quarter = 3 AND school = 'Lasip Grande Elementary School'";
