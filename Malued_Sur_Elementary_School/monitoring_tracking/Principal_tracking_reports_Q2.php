...
if (mysqli_num_rows($result_adviser) > 0) {
    ...
    while ($row_adviser = mysqli_fetch_assoc($result_adviser)) {
        ...
        $unupdated = 0;
        foreach ($tables as $table) {
            $sql_unupdated = "SELECT COUNT(DISTINCT lrn) AS unupdated_count FROM $table WHERE grade = '{$row_adviser['grade']}' AND section = '{$row_adviser['section']}' AND school = 'Malued Sur Elementary School' AND quarter = 2 AND gname != ''";
